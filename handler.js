    ...
    try {
        ...
        Client.cmd.on('bc', async (data) => {
            ...
            list.forEach(async dataC => {
                if(mediaBuffer) Client.sendFileFromBase64(dataC.jid, mediaBuffer.toString('base64'), `bc.${ext}`, `*LeviHS Broadcast*\n\n${data.body} ${dataC.jid.endsWith('@g.us') ?'\n\n_#levihs_store_*'+dataC.name+'*_' : ''}`)
                else Client.sendText(dataC.jid, `*LeviHS Broadcast*\n\n${data.body}\n\n_#levihs_store_*${dataC.name}*_`)
            })
        })
            ...
            switch(command.toLowerCase()) {
                ...
                case 'ytsearch':
                    try {
                        ...
                        await Client.sendFileFromUrl(from, ttt[0].video.thumbnail_src, 'p.jpg', teks, message)
                    } catch {
...
`
                ...
                case 'stickermenu':
                    ...
                    break
                case 'tes':
                    data.reply('Botnya masih aktif mank')
                    break
                case 'return':
		        case 'eval':
                    if(!data.isOwner) return data.reply(mess.ownerOnly)
